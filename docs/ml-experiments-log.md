...
Точка отсчёта — модель **v3.1** (EfficientNet-B0, 8 классов:
`airy, dark, dramatic, golden_hour, minimalist, monochrome, neon, vintage`,
val acc = 0.694, macro F1 = 0.687, воспроизводимый split). См. `docs/training-metrics.md` и M19 в `docs/work-log.md`.

## Ключевой аргумент защиты — устойчивость к перетасовке split'а

Готовая формулировка для §5.2 / §6 / устного выступления.

При воспроизведении split'а оригинальной v3 на честном (без data leakage)
варианте val_acc составила 0.694 против 0.726 — разница 3 пп лежит в
пределах статистической дисперсии. **Главное наблюдение**: per-class F1
изменились в диапазоне −9…+3 пп, при этом:

- Иерархия сложности классов **полностью сохранилась**:
  `neon` > `golden_hour` > `airy`/`vintage` > `dark`/`minimalist` > `dramatic` ≈ `monochrome`.
- Топ-путаницы **те же**: `monochrome` ↔ `minimalist`, `dramatic` ↔ `dark`/`monochrome`.
- Bias-distribution **улучшилась**: ratio предсказанных/истинных в [0.94, 1.03] против [0.87, 1.08] в v3.

**Вывод для защиты:**

> Модель устойчива к перетасовке валидационной выборки: изменения
> per-class F1 не превышают 9 процентных пунктов между независимыми
> обучениями на одном датасете, при этом общая структура ошибок и
> иерархия сложности классов сохраняются. Это подтверждает, что
> наблюдаемые закономерности — методологические свойства задачи и
> таксономии (операциональная определимость `neon`/`golden_hour`,
> ортогональность `monochrome` к композиционным классам, концептуальная
> близость `dark`/`dramatic`/`monochrome` в B&W high-contrast подмножестве)
> — это **свойства датасета и архитектуры**, а не артефакты конкретного
> train/val split'а.

Этот аргумент — **сильнее** простого упоминания одной точки accuracy:
показывает, что автор понимает разницу между удачным запуском и
методологически устойчивым результатом. Это требует от рецензента
принять выводы по существу, не отделываясь критикой «а если бы split
был другой?».
